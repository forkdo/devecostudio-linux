...
The exact IDEA version is **derived from the Mac DMG**, not set by hand:
`prepare()` reads the Mac `buildNumber`, finds the IDEA release it matches,
and downloads the **latest patch** of that branch (e.g. Mac built against
`2026.1.1` → package uses `2026.1.5`). There is no `-a / --ideaver` argument
anymore.

#### Building from a local IDEA tarball (`--idea`)

If you already have the matching IntelliJ IDEA tarball, you can skip the
automatic download and build from it. Name it `idea-<version>.tar.gz`
(e.g. `idea-2026.1.5.tar.gz`) and place it next to the PKGBUILD, then enable
local mode:

    _use_local_idea=true makepkg -si

`prepare()` extracts the Mac DMG and prints, before the IDEA step:

    DevEco Studio version : 26.0.0.621
    Mac buildNumber      : 261.23567.138.36.2600621
    Upstream IDEA baseline: major=261  branch=261.23567.138

It then **audits consistency** between that IDEA baseline and the IDEA tarball's
`build.txt`. If they disagree, the build aborts with a mismatch error — this
catches the common mistake of pairing a Mac DMG with the wrong IDEA snapshot
(only the JBR/native libs would be mismatched, producing a broken package).
Override the check with `_idea_force=true` if you know what you're doing.
...
3. The IDEA base resolves automatically from the Mac DMG's `buildNumber` — you normally don't need to set it by hand. To pin a local IDEA tarball instead, use `_use_local_idea=true` (see above).
